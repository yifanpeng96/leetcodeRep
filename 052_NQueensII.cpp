...
void backtrack(int& count, int n, int row, unordered_set<int>& columns,
	...
	if (row == n) {
		...
		for (int i = 0; i != n; ++i) {
			...
			//继续在 row + 1 行上放皇后
			backtrack(count, n, row + 1, columns, diagonals1, diagonals2);
...
int totalNQueens(int n) {
	...
	backtrack(count, n, 0, columns, diagonals1, diagonals2);
	return count;
}
